...
public class EnhancedController extends Controller {
     ...
     * @throws FormValidationException In case any validation fails. Error message will be added to the form.
     */
    public static <T> void handleSaveForm(Form<T> form) throws FormValidationException {
        handleSaveForm(form, null);
    }
     ...
     * @param form             The form to be validated.
     *                         Example: Form<User> userForm = form(User.class).bindFromRequest();
     * @param overrideDbErrors messages to override default persistence errors
     * @param <T>              The Model associated with the form (User.class in the given example)
     * @throws FormValidationException In case any validation fails. Error message will be added to the form.
     */
    public static <T> void handleSaveForm(Form<T> form, Map<PersistenceErrorHandler.CODES, String> overrideDbErrors) throws FormValidationException {
        if (form.hasErrors()) {
            ...
            if (invalid == null) {
                ...
                try {
                    ...
                    addPersistenceError(form, pe.getCause(), overrideDbErrors);
                    throw new FormValidationException();
    ...
    /**
     * Avoid exception using form.get() before checking if form has errors.
     * Used to ease building custom messages when saving forms
     *
     * @param form The form to be used
     * @param <T>  Model class wrapped in form
     * @return Model <T>
     * @throws FormValidationException if form is has errors
     */
    public static <T> T safePullModel(Form<T> form) throws FormValidationException {
        if (form.hasErrors()) {
            throw new FormValidationException();
        }
        return form.get();
     ...
     * buildErrorMap( "DUPLICATE_PK", "Duplicate email",
     ...
     */
    public static Map<PersistenceErrorHandler.CODES, String> buildErrorMap(String... args) {
